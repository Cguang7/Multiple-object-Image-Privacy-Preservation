...
def sample(cfg, logger):
    ...
    path = "E:/data/cropface"#'dataset_samples/faces/pioneer256x256'

    paths = list(os.listdir(path))
    paths.sort()
    paths_backup = paths[:]

    attribute_values = [float(0) for i in indices]
    ...
    def update_image(w, latents_original,curfile):
        with torch.no_grad():
            ...
            imtest=resultsample.type(torch.uint8)
            img = tensor_to_PIL(imtest)
            img.save('E:/data/fake/'+curfile)
    ...
    set_att_values = [0,5,5,5,5,5,5,5,5]

    new_latents = latents + sum([v * w for v, w in zip(set_att_values, W)])

    update_image(new_latents, latents_original,imfile)
